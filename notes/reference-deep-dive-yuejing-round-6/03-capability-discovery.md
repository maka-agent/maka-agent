# 03 — Reference app model capability discovery: 4-tier resolution
...
> covers HOW reference app knows which models support vision, function
...
Why does reference app care? Each capability gates behavior:
...
If nothing is known about a model, reference app assumes it streams.
...
out of the request. Without these overrides, reference app would pass
...
auth. Reference app fetches once per 10 minutes when cache stale.
...
Reference app transforms via `_c(t)` (a normalizer) into its own
...
function calling reliably even though `gpt-4o` does. Reference app
overrides post-hoc.
...
shape evolves (reference app added `functionCallingViaXml` and
...
- The `models.dev` schema `_c(t)` normalizer isn't traced.
  What fields does models.dev expose vs reference app's internal shape?
