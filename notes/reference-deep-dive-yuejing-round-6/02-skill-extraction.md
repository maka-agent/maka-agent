# 02 — Reference app skill extraction: the self-evolution meta-loop
...
> mentioned skill extraction as the meta-loop where reference app watches
...
than creating duplicates. Whether reference app actually wires up "update
...
the rest of the events use (round-4 [`07-websocket-sync.md`](../reference app-deep-dive-yuejing-round-4/07-websocket-sync.md)).
...
- The actual skill creation path (after `worthy === true`) isn't
  ...
  does the new SKILL.md get written? `~/.config/reference app/skills/<name>/SKILL.md`
  per round-3 01.
- The `existingSkillToUpdate: "name"` output suggests an update
  path. Does reference app diff-merge into the existing SKILL.md, or
