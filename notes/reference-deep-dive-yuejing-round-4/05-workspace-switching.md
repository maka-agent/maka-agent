# 05 — Reference app workspaces: schema, default resolution, project scoping

> Source-grounded against `~/Downloads/reference-source/readable/main.js`.
> Rounds 1-3 never traced this. Round-3 [`01-skills-system.md`](../reference app-deep-dive-yuejing-round-3/01-skills-system.md)
> noticed `.reference app/skills` was a workspace-scoped path but didn't
...
- **Worktrees are workspaces too.** A git worktree gets its own
  ...
  mode in reference app's own Workflow tool documentation.
- **PR metadata is baked in.** `prNumber` / `prUrl` / `prState` /
  `prBaseBranch` on the workspace row directly. Reference app can show a
...
Reference app resolves "which workspace?" for any new thread or bot
...
run, where Read/Edit/Write default to, what `.reference app/skills` scope
loads.

`artifactWorkspaceId` is the **artifact storage** for chat-
generated files (code snippets the agent wrote, output files,
etc.). When `enableArtifacts` is set on a thread, reference app lazily
...
| Scope | Path | Purpose |
|---|---|---|
| Skills | `<workspace>/.reference app/skills` | Project-specific skills (round-3 01). Loaded when workspace is open. |
| Agent profiles | `<workspace>/.reference app/agents` (inferred from cross-refs in round-2 07) | Per-project agent overrides. |
...
`.reference app/skills` is treated equivalently to `~/.config/reference app/skills`
for safety checks.
...
`autoWorktree: true` on a workspace tells reference app: "when an agent
...
parent. `worktreeBranch` records the branch name; reference app can
...
Open question: does reference app cleanup unused worktrees? Without
...
- Worktree lifecycle: does reference app auto-cleanup unused
...
- PR metadata on the workspace row implies reference app watches `gh pr
...
- Round 3: [`01-skills-system.md`](../reference app-deep-dive-yuejing-round-3/01-skills-system.md)
  — `<workspace>/.reference app/skills` is the 6th skill root, only
