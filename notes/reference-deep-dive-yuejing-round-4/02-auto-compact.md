# 02 — Reference app autoCompact: three trigger sites + three-tier fallback
...
reports 64k max output, reference app reserves only 32k. Models that
...
- The `<context_summary>` stripping at `HE` only handles
  reference app-generated tags. What if the user pasted markup that
