# 03 — Reference app memory recall: Recall + OperateMemory + autoRetrieve pipeline
...
Three separate model slots — reference app encourages users to pick
...
reference app decouples query language from storage language.
...
provider chain — reference app has its own embedding provider abstraction
...
Open question: does reference app re-embed on model change, or just
...
**linked** through reference app's people-profile system (`reference app people
...
This means a fact you told reference app in Telegram surfaces when you
...
2. **Lower default similarity threshold to ~0.2–0.3 if query
   rewriting lands.** The 0.1 reference app uses is too aggressive
...
- Does reference app re-embed memories when the user switches embedding
...
- Round 4: [`01-rest-api-operator-agent.md`](./01-rest-api-operator-agent.md)
  ...
  surface that lets the renderer (and app-operator agent) hit
