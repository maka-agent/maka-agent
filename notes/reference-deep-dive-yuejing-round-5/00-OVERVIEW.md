...
| 01 | `01-acp-bridge.md` | ACP provider integration via @mcpc-tech/acp-ai-provider + setSessionUpdateHandler wrapper + tool_call_update + permission delegation back to reference app's lh ladder | **shipped** |
...
| 07 | `07-provider-abstraction.md` | Universal `hd()` factory dispatching 14 non-ACP provider types + 5-pattern taxonomy (OpenAI direct / OpenAI-compatible / Anthropic-compatible / Azure-special / Aggregator-with-tag / Dynamic-auth) + universal `dd` proxy-aware fetch hook + copilot-only SDK caching for dynamic auth + aihubmix APP-Code header + custom max_tokens→max_completion_tokens wrapper + claude-subscription cloak fetch | **shipped** |
