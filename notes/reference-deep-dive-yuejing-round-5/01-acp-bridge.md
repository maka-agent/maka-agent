# 01 — Reference app ACP bridge: delegating to Claude Code / Codex / Cursor / Antigravity

> Source-grounded against `~/Downloads/reference-source/readable/main.js`.
> Round-2 [`04-permissions-runtime.md`](../reference app-deep-dive-yuejing-round-2/04-permissions-runtime.md)
...
> traces the full ACP bridge: how reference app plugs in external coder
> agents as if they were native providers, and how their tool
> calls + permission requests bridge back to reference app's UI.

## What ACP is

**ACP = Agent Client Protocol.** A JSON-RPC dialect spoken by
external "coder agent" runtimes that reference app launches as
...
The schema (from round-4 [`01-rest-api-operator-agent.md`](../reference app-deep-dive-yuejing-round-4/01-rest-api-operator-agent.md)
...
```typescript
{
  ...
  acpModelMapping?: {          // map reference app model ids → bridge model ids
    defaultModel?: string,
...
subscription, point an reference app provider at it, and reference app's agent
loop uses Claude Code as the inference backend AND uses
Claude's tool model. This is critical for users wanting to
USE their Claude Code subscription quota from inside reference app.

## Session lifecycle

ACP sessions are keyed on `(providerId, threadId)` at
`main.js:48110-48150` (round-4 [`05-workspace-switching.md`](../reference app-deep-dive-yuejing-round-4/05-workspace-switching.md)
...
- `config.requestThreadId`: the reference app thread that initiated this
...
client.setSessionUpdateHandler = (downstreamHandler) => {
  original(notification => {
    // INTERCEPT for reference app's bookkeeping
    if (notification.update?.sessionUpdate === "available_commands_update") {
      session.availableCommands = notification.update.availableCommands;
      // notify reference app-side listeners (Settings UI, etc.)
    ...
    if (notification.update?.sessionUpdate === "tool_call_update") {
      // synthesize a tool-execution-result event for reference app's UI
...
This is a **subscription proxy**: reference app sees every ACP event,
processes the ones it cares about (commands list, tool calls),
then forwards the same event to whatever the AI SDK adapter
expected. The AI SDK never knows reference app is reading along.
...
return true` check. Without this, reference app's interception layer
would stack — every event would fire reference app's callbacks N times.
...
the console would drown real errors. Reference app matches against both
...
agent runs — Read, Write, Bash, etc. Reference app rebroadcasts each as
...
in real-time — same UX as if reference app's own Read tool ran. The user
...
- **`status === "failed"`** → `isError: true`: normalized to
  reference app's boolean. Other statuses (pending, completed) pass
  through as-is.

## Permission delegation — ACP asks reference app's user
...
`setPermissionRequestHandler`. Reference app delegates back to its OWN
permission UI:
...
client.setPermissionRequestHandler(async (req) => {
  ...
  // Tell reference app's UI: ACP needs permission
  this.toolExecutionResultCallback?.({
    toolCallId, toolName, status: "permission-requested", threadId, providerId,
  });

  // Use reference app's own lh() permission ladder (round-3 04)
  ...
  // Map reference app decision → ACP option
...
execution context**. If reference app's parent agent invoked a Task
...
So **reference app's permission system has built-in awareness of "this
...
- `status: "permission-requested"` — modal opens, reference app's UI
...
**every reference app feature that consumes models works with ACP**:
...
- Even the app-operator agent (round-4 01) could theoretically
  be backed by ACP

This is the architectural payoff: by encapsulating ACP behind
the SDK provider abstraction, reference app gets near-zero integration
...
- No `setPermissionRequestHandler` delegation back to reference app's UI
...
- The `acpMcpServerIds` field forwards Maka MCP servers to the
  ACP runtime. How? Does reference app spawn `claude` with extra args,
...
- The `acpModelMapping` has `opusModel`/`sonnetModel`/
  `haikuModel`/`subagentModel` slots — does reference app route by Task
  weight or by reference app's own subagent system (round-2 07)?
...
- The `acpAuthMethodId` field implies multi-auth support — does
  reference app know how to OAuth Claude Code, or does it delegate
...
- Round 4: [`01-rest-api-operator-agent.md`](../reference app-deep-dive-yuejing-round-4/01-rest-api-operator-agent.md)
  ...
  can manage via curl.
- Round 4: [`05-workspace-switching.md`](../reference app-deep-dive-yuejing-round-4/05-workspace-switching.md)
