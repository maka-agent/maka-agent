# 07 — Reference app provider abstraction: 16 types behind one factory
...
EVERY agent loop in reference app — autoCompact summary model (round-4
...
Without a universal hook, reference app would have to thread proxy
...
Anthropic's protocol, so reference app reuses Anthropic SDK.
...
and it requires `apiVersion`. Reference app extracts the resource name
...
revenue-share tracking. The literal `"OJQA1051"` is **reference app's
own APP-Code** — meaning aihubmix attributes reference app users to reference app
...
This is the **decision tree reference app's `hd()` implements
...
What's missing (compared to reference app):
...
- The `claude-subscription` cloak's `sessionKey: "reference app-default"`
...
- New providers from the AI SDK ecosystem (e.g., Cerebras,
  Mistral) — does reference app add them via `custom` or wait for
  dedicated SDKs?

## Cross-refs

- Round 2: [`09-cloak-request-full.md`](../reference app-deep-dive-yuejing-round-2/09-cloak-request-full.md)
...
- Round 5: [`06-telemetry-sentry.md`](./06-telemetry-sentry.md)
  — Sentry's HTTP instrumentation is automatic ONLY if reference app's
