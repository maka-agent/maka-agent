...
Yetone's `f = async () => { … }` opens at `~/Downloads/reference-source/readable/main.js:62100`. Per `~/Downloads/reference-source/docs/01-agent-loop.md §1`, the locals at entry are:
...
Three responsibilities, all in `~/Downloads/reference-source/readable/main.js:62944-63096`. Per-step closure that returns `{activeTools?, messages?}`.
...
The single biggest cost-saver for Anthropic. From `~/Downloads/reference-source/readable/main.js:50566-50620`:
...
Per `~/Downloads/reference-source/docs/01-agent-loop.md §19 insight 8`: "Cache markers stamped on exactly 4 messages. Combined with the SYSTEM INFO splitter, you get 4 cacheable Anthropic breakpoints (the SDK's max)."
...
- **`Mt = !!ne && Object.keys(ne).length > 0` toggles external reference wrap (`main.js:62914`)** — Maka doesn't need OS-model XML parsing.
