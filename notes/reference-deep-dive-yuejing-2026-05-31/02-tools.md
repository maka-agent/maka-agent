...
**Only Bash calls `lh()`.** Write, Edit, ChromeRelay*, BrowserEval, etc. trust the model. The justification is "the parent chat session has already approved Reference app at install time."
...
3. **Auto-promote**: after 60s, the still-running process is *silently* promoted to a background shell (`bash_id` returned to the model). "Unique to Reference app — Claude Code's Bash kills, Reference app keeps it alive" (per `02-tools.md §1`).
...
Model sees a 1024px-wide JPEG thumbnail in chat context; full-resolution JPEG goes to `os.tmpdir()/reference app-chrome-screenshot-full-<ts>.jpg` and the path is in the result so a follow-up Bash can re-read it.
...
`main.js:26302-26472` (full text in `~/Downloads/reference-source/docs/02-tools.md §12`).
...
Per `~/Downloads/reference-source/docs/02-tools.md §MCP tools`: MCP is **not registered into `vw`**. The merge happens in the chat handler at `main.js:61222`:
...
The 3 transports + OAuth 2.1 PKCE are in `~/Downloads/reference-source/docs/09-mcp.md`. For Maka this is future work; not urgent.
...
- `app-guide` — answers about the Reference app app itself
- `app-operator` — reads/writes Reference app config
...
Per `~/Downloads/reference-source/docs/00-GAP-ANALYSIS.md`, `mu` is **NOT** PostHog telemetry — it's the *Plugin hook lifecycle wrapper* that emits `tool.willExecute`/`tool.didExecute` hooks. The naming is misleading.
