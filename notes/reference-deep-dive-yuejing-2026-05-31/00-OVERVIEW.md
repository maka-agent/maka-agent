...
- `better-sqlite3` + Drizzle (typed query builder only, raw `CREATE TABLE` DDL) at `~/Library/Application Support/reference app/chat_threads.db` — 49 logical tables incl. `sqlite-vec` `memory_embeddings` and `messages_fts`.
- Markdown "soul tree" at `~/.config/reference app/` (SOUL.md / USER.md / MEMORY.md / HEARTBEAT.md / diary / people / chats / groups / selfies / skills / plugins).
- macOS Swift `ReferenceComputerUse.app` daemon over `~/Library/Application Support/Reference app/computer-use.sock` for AX + CGEvent control.
...
- A separate `reference app` CLI binary (Bun-compiled, ~150 sub-commands) that is how skills reach the running app — every skill calls `reference app foo` which translates to `http://localhost:23001/api/...`.
...
## 2. Repository layout in `~/Downloads/reference-source/`
...
└── docs/                                # 38 deep-dive Markdown notes, ~55,409 lines, 2.6 MB
    ...
    ├── 17-workspaces-git.md             # auto-worktree + gh CLI + .reference app-snapshots/
...
```
                ...
                └─┬─────────┬───────────┬────────────┬────────────┬─────────┘
                  │         │           │            │            │
                  ▼         ▼           ▼            ▼            ▼
            Renderer    Bundled      ReferenceCU       Chrome       reference app CLI
...
Citation: `~/Downloads/reference-source/readable/main.js:62944-63096`; analysis at `~/Downloads/reference-source/docs/01-agent-loop.md §7`.
...
Citation: `~/Downloads/reference-source/readable/main.js:24058` (Read), `27865` (BrowserScreenshot), `28677-28741` (ChromeRelayScreenshot); `02-tools.md §17-27`.
...
Citation: `~/Downloads/reference-source/readable/main.js:50582-50620` (`wk()` helper) and `62807-62830` (callsite); `01-agent-loop.md §2`.
...
Citation: `~/Downloads/reference-source/readable/main.js:50289-50335` (`nk()` watchdog generator), `63113` (closure init), `62945, 63130, 63147, 63120, 63267` (resetters); `01-agent-loop.md §11`.
...
Citation: `~/Downloads/reference-source/readable/main.js:28877-28906`; `02-tools.md §40`.
...
Citation: `~/Downloads/reference-source/docs/03-prompts.md §2-§4`, `08-memory.md §7`, prompt assembly at `main.js:61711, 61679, 61741, 61897-62141`.

Yetone stores three classes of long-term identity in user-editable Markdown:

- **SOUL.md**: AI's own self-concept; only the `## Evolved Traits` section is append-only by the AI via `reference app soul append-trait`.
...
Citation: `~/Downloads/reference-source/readable/main.js:24627-24684`; `02-tools.md §11`, `04-skills.md`.
...
Citation: `~/Downloads/reference-source/readable/main.js:22272-22478` (Task tool execute), `21900-ish` (mission orchestrator); `02-tools.md §9`, `08-memory.md §3.5`.

When `handoff.harness.enabled === true`, the Task tool routes to a Planner→Generator→Evaluator loop persisted in 6 SQLite tables: `agent_missions`, `agent_runs`, `agent_handoffs`, `mission_sprints`, `sprint_contracts`, `sprint_evaluations`. Per-mission `spec_artifact_path` writes a sidecar markdown under `~/.config/reference app/missions/`.
...
Citation: `~/Downloads/reference-source/readable/main.js:24254-24506` (Hm = hash-line Edit) and `24131-24252` (jm = classic search/replace); `02-tools.md §6`.
...
Citation: `~/Downloads/reference-source/readable/main.js:19345-19400`; `02-tools.md §0 (Approval architecture)`.
...
| Topic | Primary doc |
|---|---|
| Agent loop (streamText, retries, watchdog) | `~/Downloads/reference-source/docs/01-agent-loop.md` |
| Tool registry, approval gate, streaming hooks | `~/Downloads/reference-source/docs/02-tools.md` |
| All 47 prompt sites verbatim | `~/Downloads/reference-source/docs/03-prompts.md` |
| 33 bundled SKILL.md packages | `~/Downloads/reference-source/docs/04-skills.md` |
| 238 Express routes (security audit) | `~/Downloads/reference-source/docs/05-express-api.md` and `~/Downloads/reference-source/docs/51-express-handlers.md` |
| Computer Use over UDS | `~/Downloads/reference-source/docs/06-computer-use.md` |
| Chrome MV3 + CDP relay | `~/Downloads/reference-source/docs/07-chrome-extension.md` |
| SQLite + sqlite-vec + soul tree | `~/Downloads/reference-source/docs/08-memory.md` |
| MCP integration (3 transports, OAuth) | `~/Downloads/reference-source/docs/09-mcp.md` |
| `reference app` CLI verbs + Ink TUI | `~/Downloads/reference-source/docs/10-cli-tui.md` |
| 17 provider types + reasoning | `~/Downloads/reference-source/docs/11-providers.md` |
| Multi-window architecture, IPC bridge | `~/Downloads/reference-source/docs/12-renderer.md` |
| Activity Recorder, Sentry, updater | `~/Downloads/reference-source/docs/13-aux.md` |
| PinchTab vs ChromeRelay engines | `~/Downloads/reference-source/docs/14-browser-engines.md` |
| 384 renderer chunks decoded | `~/Downloads/reference-source/docs/15-renderer-chunks.md` |
| Telegram / Discord / Feishu / WeChat | `~/Downloads/reference-source/docs/16-bots.md` |
| auto-worktree + `.reference app-snapshots/` | `~/Downloads/reference-source/docs/17-workspaces-git.md` |
| Prompt apps + slash commands | `~/Downloads/reference-source/docs/18-prompt-apps.md` |
| Cron / heartbeat / fatigue | `~/Downloads/reference-source/docs/19-time-driven.md` |
| ACP coding-agent subagents | `~/Downloads/reference-source/docs/20-acp.md` |
| Skill install + skillflag CLI | `~/Downloads/reference-source/docs/21-skill-install.md` |
| Embeddings, sleep loop | `~/Downloads/reference-source/docs/22-embeddings.md` |
| Bash safe/dangerous regex | `~/Downloads/reference-source/docs/23-bash-sandbox.md` |
| Telemetry / Sentry / auto-update | `~/Downloads/reference-source/docs/24-telemetry.md` |
| Native modules | `~/Downloads/reference-source/docs/25-native-modules.md` |
| Plugins system | `~/Downloads/reference-source/docs/30-plugins.md` |
| All 44 LLM call sites | `~/Downloads/reference-source/docs/31-llm-calls.md` |
| Companion state machines | `~/Downloads/reference-source/docs/32-companion-state.md` |
| Hooks + cookies | `~/Downloads/reference-source/docs/33-hooks-cookies.md` |
| TUI walkthrough | `~/Downloads/reference-source/docs/35-tui-modules.md` |
| Renderer entry points | `~/Downloads/reference-source/docs/53-renderer-entries.md` |
| Boot sequence | `~/Downloads/reference-source/docs/49-boot-sequence.md` |
| Settings tree | `~/Downloads/reference-source/docs/50-settings-tree.md` |
| Cross-doc corrections | `~/Downloads/reference-source/docs/00-GAP-ANALYSIS.md` |
...
5. The original `~/Downloads/reference-source/docs/01-agent-loop.md §19` "Key insights & non-obvious tricks" — 20 numbered insights, all citable.
...
- **Bundled skill override is asymmetric** — `~/.codex/skills/` and project `.reference app/skills/` can override a bundled name; `~/.config/reference app/skills/` and `~/.claude/skills/` *cannot*. This is per `docs/21-skill-install.md §1` and is probably a bug, not an intentional design. If Maka mirrors this, mirror the symmetric version.
