...
> Sixth pass through the reference app reverse-engineering output. Covers topics that
> notes 00-07 only mentioned in passing. Sources for every citation:
>
> - `~/Downloads/reference-source/docs/<NN>-*.md` (the reference app re-eng docs themselves)
> - `~/Downloads/reference-source/readable/main.js` (74,182 lines, prettified)
>
...
1. **MCP**. reference app is a full MCP **client** with three transports
   (stdio / streamable HTTP / SSE) + OAuth 2.1 + PKCE + DCR + safeStorage
   tokens. Config lives in `~/.config/reference app/mcp.json`; per-server tokens in
...
4. **ACP**. reference app drives external coding agents (Claude Code, Codex CLI,
   Gemini, OpenCode) via Agent Client Protocol (JSON-RPC over stdio). Has a
   `ToolProxyHost` TCP server that lets the sub-agent reach back into reference app's
...
5. **Embeddings**. sqlite-vec virtual table + dispatcher across OpenAI /
   ...
   MEMORY.md.
6. **Bash sandbox**. There isn't one. reference app's defense is regex pre-filter +
...
- **Imports**, `main.js:179-193`: SDK pulled from
  ...
  `~/Downloads/reference-source/docs/09-mcp.md §1`.
- **Source of truth at runtime**: file
  `~/.config/reference app/mcp.json` (Claude-Desktop-compatible shape:
...
- **Token storage**: tokens + refresh + client_secret + code_verifier
  ...
  (`main.js:24730-24739`). Cited `~/Downloads/reference-source/docs/09-mcp.md §3`.
...
- **OAuth flow** (`main.js:24878-25185`): app-level manager `gf` opens a
  ...
  during DCR identifies as `{ client_name: "Reference app Desktop",
  ...
  `~/Downloads/reference-source/docs/09-mcp.md §5`.
...
- **For ACP sessions** (`main.js:19740-19757`): reference app forwards a declarative
  list of MCP server configs to the spawned coding agent so the agent
  spawns its own MCP clients, instead of multiplexing through reference app.

### 1.2 Why it exists

MCP is the de facto inter-tool protocol that lets users plug in third-party
tool servers without recompiling reference app. The JSON-file-as-source-of-truth
...
- The closest analog to the reference app OAuth manager is in
...
- **B-MCP-01** *(Tier 1)* — Ship the JSON-config + tool-name prefix
  ...
  `~/.config/maka/mcp.json` with the reference app schema (Claude-Desktop-compatible
...
- **B-MCP-03** *(Tier 2)* — Stdio transport only: implement
  ...
  JSON-RPC framing. Reuse the reference app timeout matrix (60s connect / 15s
...
- **B-MCP-04** *(Tier 3)* — Tool-call permission gate. Unlike reference app, route
  ...
  reference app deliberately bypasses approval at `main.js:19228-19253`; we should
  not. Scope: S.
...
- **Storage**: SQLite table `prompt_apps`, Drizzle model `bn` at
  ...
  `~/Downloads/reference-source/docs/18-prompt-apps.md §1.2`.
...
- **Global hotkey** (`main.js:72918-72988`): `pA(promptAppId,
  ...
  (`"Cmd+Shift+J"`).
- **What it is NOT**: no CLI surface (`reference app prompt-app run` doesn't
  exist), no filesystem backing (only SQLite rows), no bundled default
  apps, no internal usage by title-gen/compaction. Per
  `~/Downloads/reference-source/docs/18-prompt-apps.md §1.11-1.13`.
...
- Maka does ship a single-prompt skill mechanism through
  ...
  reference app's "skills" than "prompt apps".
...
- **B-PAPP-03** *(Tier 2)* — Promote Maka's existing Quick Chat window
  ...
  `webContents.id` → map keyed lookup (mirrors reference app's `SS` map at
...
- **B-PAPP-06** *(Tier 4)* — Execution-history table (separate from the
  ...
  "Re-run with same inputs". Mirrors reference app's `prompt_app_executions` at
...
- **Singletons constructed during Express startup**: `HeartbeatService`
  ...
  `~/Downloads/reference-source/docs/19-time-driven.md §1`.
...
- **Cron storage** at `~/.config/reference app/cron/jobs.json` and `runs.json`.
...
- **HeartbeatService** (`hv`, per `~/Downloads/reference-source/docs/19-time-driven.md
  §3`): periodic poke every `intervalMinutes` (default 30). Reads
  `~/.config/reference app/HEARTBEAT.md`, composes a multi-section prompt
...
- **Fatigue / sleep state machine** lives in
  ...
  `~/.config/reference app/fatigue.json`. Continuous decay (0.8/min idle) + 1.5
...
1. **Agent-as-roommate**. reference app is positioning itself as a long-running
...
2. **One code path** for scheduled work and inbound messages. By routing
   cron through the same WebSocket the chat engine listens on, reference app
...
- **B-TIME-02** *(Tier 2)* — Two execution modes: `isolated` creates a
  ...
  `main` injects into an existing session. Mirrors reference app's
...
- **B-TIME-03** *(Tier 2)* — Cron-stuck-generation cleanup timer
  ...
  `main.js`. Skip reference app's 480s pre-flight kill for v1 — too aggressive.
  Scope: S.
...
### 4.1 reference app's design

- **Protocol**: JSON-RPC 2.0 over child-process stdio, newline-delimited.
  Three roles: reference app is the **client**; an external coding agent
  (claude-code-acp, codex-acp, gemini, opencode, pi) is the **agent**.
  Per `~/Downloads/reference-source/docs/20-acp.md §1`.
- **Method names seen**: `initialize`, `authenticate`, `session/new`,
  ...
  client). `~/Downloads/reference-source/docs/20-acp.md §1` (method table).
...
- **Three execution paths for the `coder` subagent**
  ...
  1. **Built-in** — in-process `streamText` with reference app's tools
...
- **ToolProxyHost** (`@mcpc-tech/acp-ai-provider/index.mjs:152-`): when
  ...
  **TCP server** on a random port and pushes an
  ...
  reference app's host. So the ACP child can directly use the parent's host
...
- **Env injection for the spawned child** (`main.js:21084-21156`):
  reference app reverse-shims the LLM API. For `claude-code-acp` linked to an
  OpenAI provider, set
  `ANTHROPIC_BASE_URL=http://localhost:${WhPort}/anthropic-proxy/${providerId}`
  so the child speaks Anthropic Messages API to reference app's local server,
...
- **MCP forwarding via ACP** (`main.js:19740-19757`): reference app builds a
...
- **Permission posture**: `acpx --approve-all` and Claude Code
  ...
  bypass reference app's modal. Doc §4 explicitly flags this as risky.

### 4.2 Why it exists

reference app wants users to bring whatever coding agent they already use
...
- **No retries, no batching, no queue** in the embedding layer. One
  HTTP request per call. Per `~/Downloads/reference-source/docs/22-embeddings.md
  §2`.
...
- The data structure is per `local-memory.ts:18-78`:
  ...
  reference app's `temporary` durability but isn't used yet.
...
- **B-EMB-04** *(Tier 4)* — Nightly four-layer sleep cycle. Schedule
  ...
  with no synthesis (mirrors reference app's check). Scope: L. Pre-req: EMB-01
...
## 6. Bash sandboxing model (and why reference app calls it that)

### 6.1 reference app's design

- **There is no sandbox**. Per `~/Downloads/reference-source/docs/23-bash-sandbox.md
...
- **Spawn details**:
  ...
  - **Three concurrent watchdogs** on foreground (`main.js:23389-23407`):
    ...
    2. **Idle watchdog**: every 10s, if no stdout/stderr for >90s, mark
       ...
       `[reference app] Process killed: no output for 90s (likely waiting for
...
- **`Wd()` rtk rewrite** (`main.js:17817`): if first non-assignment token
  ...
  command with `<rtkPath>/rtk `. `rtk` is reference app's tracking helper that
  logs dev-tool invocations to `<userDataDir>/reference app/rtk-tracking.db`.
...
approval + cached policy. reference app's contribution is making the speed-bump
...
- **B-SBX-04** *(Tier 1)* — Auto-promote-to-background after 60s. New
  ...
  `KillShell` tools (mirrors `~/Downloads/reference-source/docs/23-bash-sandbox.md
...
| 5 | **B-SBX-04** Bash auto-promote-to-background + `BashOutput` | M | Today every long-running command (build, test, dev server) blocks the turn until 120s timeout. Adding background promotion lets the LLM start a build, do other things, then check back. Matches reference app's `main.js:23389-23407` design. |
...
## Appendix: reference app source citations index
