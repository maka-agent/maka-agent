...
Per `~/Downloads/reference-source/docs/12-renderer.md §1`:

| HTML file | Title | Window singleton var | Purpose |
|---|---|---|---|
| `index.html` | "Reference app" | `rS` (main), `lS` (quick-chat), `mS` (more-menu), `gS` (perm-overlay) | Main chat + 3 hash-routed siblings |
| `settings.html` | "Reference app Settings" | `cS` | Settings window |
| `gallery.html` | "Reference app Gallery" | `dS` | Selfie album / image grid |
| `lightbox.html` | "Reference app Image Viewer" | `uS` | Standalone image viewer |
| `livecoding.html` | "Reference app Live Coding" | `hS` | Strudel/Tidal live coding |
...
| `notifications.html` | "Reference app Notifications" | per `NotificationCenter` instance | Toast overlay |
...
The renderer chooses what to mount based on `location.hash`. One Vite bundle, four windows. Same pattern for `additionalArguments: ["--reference app-window-role=notifications"]` (`main.js` notification window).

### 1.1 Window configuration table

Per `~/Downloads/reference-source/docs/12-renderer.md §2.2`:
...
Per `~/Downloads/reference-source/docs/12-renderer.md §3`:
...
Per `~/Downloads/reference-source/docs/12-renderer.md §4`:
...
Per `~/Downloads/reference-source/docs/12-renderer.md §5`, 144 `ipcMain.handle(...)` registrations. The naming convention is **`namespace:action`** with hyphens-in-words and colons-between-parts:
...
Per `~/Downloads/reference-source/docs/12-renderer.md §2`:
...
Per `~/Downloads/reference-source/docs/12-renderer.md §2.2` (notifications row):
...
- `additionalArguments: ["--reference app-window-role=notifications"]` — the renderer uses this to detect "I'm the notification window" without checking URL.
...
Per `~/Downloads/reference-source/docs/12-renderer.md §2.2` (Permission Overlay row):
...
Per `~/Downloads/reference-source/docs/15-renderer-chunks.md §1`:

> The build is signed with Sentry (release id `reference app@0.0.798`) — every chunk emits a `SENTRY_RELEASE` / `_sentryDebugIds` IIFE at the top.
...
Per `~/Downloads/reference-source/docs/15-renderer-chunks.md §1`, the 37 MB of `assets/` distributes as:
...
| B-UX-12 | Verify Shiki lazy-load | `~/Downloads/reference-source/docs/15-renderer-chunks.md` | renderer | verification | — |
