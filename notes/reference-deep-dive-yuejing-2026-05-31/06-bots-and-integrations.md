...
Per `~/Downloads/reference-source/docs/16-bots.md` opening table:
...
Per-platform state files (`STATE_FILE = ~/.config/reference app/groups/state.json`) preserve `groupHistory`, `lastBotReplyTime`, `skippedMessages` across restarts.
...
Per `~/Downloads/reference-source/docs/08-memory.md §3.10`:
...
Per `~/Downloads/reference-source/docs/16-bots.md §7`:
...
Per `~/Downloads/reference-source/docs/08-memory.md §7.3` and `~/Downloads/reference-source/docs/16-bots.md §6`, USER.md and people/*.md use:
...
Per `~/Downloads/reference-source/docs/03-prompts.md §1` and `~/Downloads/reference-source/docs/16-bots.md §9`:
...
Per `02-tools.md §5.3` (and `main.js:23350-23365`): Bash env includes `ALMA_THREAD_ID` and `ALMA_CHAT_ID`. The `send-file` skill (`~/Applications/Reference app.app/Contents/Resources/bundled-skills/send-file/SKILL.md`) uses these to route attachments back to the originating chat.
...
- Per `~/Downloads/reference-source/docs/16-bots.md §5`, Yetone caches credentials at `~/.config/reference app/weixin/credentials.json` (not in settings). Maka should consider: WeChat credentials in `BotChannelSettings` are passed by value; if the user revokes the token, there's no automatic recovery path.

### 3.8 The OpenAI-compatible endpoint (`/v1/chat/completions`) (B-BOT-07)

Per `~/Downloads/reference-source/docs/01-agent-loop.md §17` and `main.js:56912`:

> OpenAI-compatible API endpoint (Reference app exposes `/v1/chat/completions`) — streams text-only chunks back as SSE for external API clients.
...
> No shared in-memory event bus — cross-platform messaging (`reference app discord send` from a Telegram thread) goes through HTTP to the Express server (e.g. `POST /api/discord/channels/:id/send`, L52238), so the originating bridge stays uninvolved.
...
Per `~/Downloads/reference-source/docs/00-GAP-ANALYSIS.md`: most of the 238 Express routes are unauthenticated. Maka has no Express. Good.
...
Per `~/Downloads/reference-source/docs/16-bots.md §11`:

- Telegram + Discord have sticker auto-indexing at `~/.config/reference app/stickers/index.json`.
...
Per `~/Downloads/reference-source/docs/16-bots.md §10`: Each bridge sends a "typing..." indicator at the platform's standard cadence while the agent is generating. Telegram: `sendChatAction(typing)` every 5s. Discord: gateway typing event. Feishu: `is_typing: true` in message payload. WeChat: no typing API.
...
| B-BOT-09 | "Typing" indicators per bridge | `~/Downloads/reference-source/docs/16-bots.md §10` | each `*-bridge.ts` | S | low |
