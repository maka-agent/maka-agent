...
Per `~/Downloads/reference-source/docs/03-prompts.md`, the chat-agent system prompt is assembled by string concatenation in the order:
...
6. **GLOBAL CONFIG** — `Be` fragment with `~/.config/reference app/...` paths.
...
Critical insight from `~/Downloads/reference-source/docs/01-agent-loop.md §2`: the system message is **split at the `"SYSTEM INFO"` line** for Anthropic/Bedrock specifically — first segment (stable bulk: personality + SOUL + USER + MEMORY + SECURITY + skills) + second segment (per-platform tail: system info + date + Christmas) become two cacheable system parts.
...
If `~/.config/reference app/SECURITY.md` exists, the contents are appended with this header. Placed very high in the prompt to outrank the personality.
...
Per `~/Downloads/reference-source/docs/31-llm-calls.md` and `~/Downloads/reference-source/docs/01-agent-loop.md §16`, Yetone has **44 LLM call sites** — 38 `generateText` + 4 `streamText` + 0 `generateObject`. Selected:

| Line | Purpose |
|---|---|
| 14860 | Telegram/IM bridge: decide voice vs text reply (`maxOutputTokens: 8`) |
| 32922 | Webpage summarizer (link preview, `maxTokens: 150`) |
| 33624 | Group-chat: should Reference app pile on with a reaction emoji |
...
| 56912 | OpenAI-compatible API endpoint (Reference app exposes `/v1/chat/completions`) |
...
`main.js:61712` — the central design choice of the entire Reference app product:

> "You are Reference app — not an assistant, not a chatbot, just... Reference app."
...
Per `~/Downloads/reference-source/docs/03-prompts.md §33-§36` — six subagent prompts, all with the same shape:
...
Per `~/Downloads/reference-source/docs/21-skill-install.md §1`, the singleton `Xu` (`main.js:18571-19126`) scans 5 roots + 2 derived:
...
projectSkillsPath          = <workspace>/.reference app/skills
...
```
process.resourcesPath/bundled-skills    ← prod (Reference app.app/Contents/Resources/bundled-skills)
...
Per `~/Downloads/reference-source/docs/21-skill-install.md §1.loadSkills`, the priority *as documented*:

```
1. bundled       — baseline, can be overridden by anything
2. personal      — ~/.config/reference app/skills
...
7. project       — <ws>/.reference app/skills     (always overrides)
```
...
`main.js:18958-19017`. On first run after the bundled-skills tree exists, walks `~/.config/reference app/skills/` and for any directory whose name matches a bundled skill compares the `SKILL.md` content. If identical (or identical modulo `always-inject:` line), `rmSync(... force:true)` — destructive delete with no user prompt. Marker `~/.config/reference app/skills/.bundled-skills-migrated` ensures one-shot.

**Avoid in Maka.** Per `~/Downloads/reference-source/docs/21-skill-install.md §1`: "a user who intentionally copied a bundled skill into `~/.config/reference app/skills/` to make a tweak, then reverted the tweak, will have the copy silently deleted on the next launch." Don't borrow this — if anything, add a tombstone file with the original sha256 and ask before deleting.
...
Per `~/Downloads/reference-source/docs/02-tools.md §11`, only **name + brief description** of each skill in the system prompt. The model sees a `<available_skills>` block with maybe 60 short entries.
...
The "force absolute paths" preamble is critical — without it, skill markdown that says "edit ~/.config/reference app/SOUL.md" might get model-fabricated into "edit ./SOUL.md" depending on cwd at invocation time.
...
`main.js:18705` reads `metadata["always-inject"]`. Per `~/Downloads/reference-source/docs/21-skill-install.md` and `~/Downloads/reference-source/docs/04-skills.md`: some skills set `always-inject: true` to be unconditionally added to the system prompt (versus on-demand via the Skill tool). Use cases: skills whose mere presence changes behavior (e.g. `plan-mode`, `self-management`).
...
`app/node_modules/skillflag/dist/install/`, version 0.1.4, MIT. Per `~/Downloads/reference-source/docs/21-skill-install.md §2`, the CLI:
...
**Reference app is not in the agent list.** The integration with `Xu` is therefore patchy (see `21-skill-install.md §2` for the full hostility of the round-trip).
...
Per `~/Downloads/reference-source/docs/21-skill-install.md §2.3`, the extractor is **commendably defensive**:
...
Per `~/Downloads/reference-source/docs/04-skills.md`, the format is plain Markdown with YAML frontmatter:
...
Per `~/Downloads/reference-source/docs/04-skills.md §0`, every skill **bottoms out** in one of three places:

1. The `reference app <command>` CLI (~28 of 33 skills primarily call it).
