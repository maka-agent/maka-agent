# 09 — Symbol Recovery: reference app `readable/main.js`

> Audit artifact, not a code change. All citations are to
> `~/Downloads/reference-source/readable/main.js:NNNN`. Names like `Xb`, `tv`, `nv`
...
- **Maka equivalent**: none (activity recorder is reference app-specific).
...
- **Inferred role**: Returns the per-user directory for Weixin bot state: prefers `app.getPath("userData")/weixin-state`, falls back to `~/.config/reference app/weixin-state` when Electron is unavailable.
- **Evidence**:
...
- **Inferred role**: Append-only logger for the Weixin bot — writes to `${tmpdir}/reference app-logs/weixin-bot-${YYYY-MM-DD}.log` and mirrors to `console.log`. Serializes objects via `JSON.stringify`.
- **Evidence**:
...
- **Inferred role**: Mirror logger used by the `MessageBridge` module — writes structured log lines to `/tmp/reference app-bridge-debug.log` and also `console.log`s. Same shape as `Vb` but a different file.
- **Evidence**:
...
- **Inferred role**: Feishu/Lark group chat log writer — per-group per-day text log under `~/.config/reference app/groups/feishu_${groupId}_${date}.log`.
- **Evidence**:
  - `main.js:41302` — `l = F.join(i, \`feishu_${e}_${a}.log\`);`
  - `main.js:41303` — `\`[${c}]${r ? ` [msg:${r}]` : ""} ${o ? "[Reference app]" : \`[${t}]\`}: …\``
...
- **Inferred role**: MCP-server registry singleton (extends `EventEmitter`/`Gt`). Holds `servers`, owns `~/.config/reference app/mcp.json`, manages `initialize`, `refresh`, `startServer`, `getAllTools`, stderr buffering.
- **Evidence**:
  - `main.js:25219` — `this.configPath = F.join(n.getPath("home"), ".config", "reference app", "mcp.json")`
...
- **Inferred role**: Discovers and serves skills from four locations: `~/.config/reference app/skills`, `~/.claude/skills`, `~/.codex/skills`, `~/.agents/skills`, plus `~/.claude/plugins`. Imports / deletes / refreshes user skills. Provides `buildSkillsContext` for prompt injection.
- **Evidence**:
