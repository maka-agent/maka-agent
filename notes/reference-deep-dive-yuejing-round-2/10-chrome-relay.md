# 10 — Reference app ChromeRelay: bridge to the user's real Chrome browser
...
`chromeRelayAuthToken` is persisted in reference app settings. The Chrome
...
token lets reference app trust that the connecting extension is the user's
...
4. **Mutual auth via persisted token.** When Maka gets ANY
   ...
   anything — the reference app pattern of `<service>AuthToken` in settings
