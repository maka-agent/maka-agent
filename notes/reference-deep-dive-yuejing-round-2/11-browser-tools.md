# 11 — Reference app built-in Browser tools (Electron-backed sandbox browser)
...
Implementation choice: reference app uses Electron's own BrowserWindow
...
- **No state until `BrowserOpen`** (`main.js:27598-27601` throws
  ...
  before opening). This is the single-instance lifecycle reference app
...
Notable details:
- **30-second hard timeout** on `loadURL`. If a page hangs, reference app
...
2. **Match reference app's `Read` / `ReadDom` PREFER over `Screenshot`
   guidance.** This is purely a tool-description choice; mirror
   the reference app copy at `main.js:27957` and the model picks the token-
   efficient option.
...
4. **Single-instance window lifecycle.** Throw a helpful error
   ...
   reference app's exact copy so the model recovers quickly when it forgets
   to open first.
...
Does reference app's Mozilla Readability extractor run in the page context
