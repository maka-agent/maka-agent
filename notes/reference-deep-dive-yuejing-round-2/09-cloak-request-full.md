# 09 — Reference app cloaked-request: every field, every block
...
> the FULL reference app shape so any latent delta is documented.
...
```js
{
  ...
  "X-Claude-Code-Session-Id": c,                              // reference app session id
...
- `Lang`: js
- `Package-Version`: 0.74.0 (the @anthropic-ai/sdk version reference app's
...
- `Retry-Count`: always "0" — reference app doesn't expose its retry chain
...
must appear first when present for Sonnet/Opus per the reference app
ordering).
...
matching `text.includes("x-anthropic-billing-header")`, reference app
...
match across turns. Like the billing header, reference app de-duplicates if
...
l.metadata = {
  ...
  user_id: JSON.stringify({
    device_id: i,            // reference app device UUID
    account_uuid: a,         // resolved from /api/oauth/profile
    session_id: c,           // per-session reference app id
  }),
};
```

`metadata.user_id` is an opaque Anthropic field that reference app uses to
...
Cross-checked Maka's `cloaked-request.ts` against the reference app
...
1. **Package version pinning.** Maka's `cloaked-request.ts:56` uses
   ...
   reference app currently ships. The version string is also part of the
   gateway's "is this a real claude-cli client" check.
2. **Runtime version**. Maka uses `process.version` while reference app
   hardcodes `'v22.13.0'`. If Anthropic's gateway has a runtime
   version allowlist, dynamic `process.version` could fail on
   newer Node releases. Worth mirroring reference app's hardcode for
   conservative parity.
