# 01 — Reference Computer Use: native helper + IPC protocol + DB

> Source-grounded against `~/Downloads/reference-source/readable/main.js`. Every
> claim cites `main.js:NNNN`. Cross-refs to round 1:
> `[../reference app-deep-dive-yuejing-2026-05-31/02-tools.md]` (tools registry).

## Architecture at a glance

Reference app's Computer Use is **not** a typescript Electron module. It's a
*separate native macOS binary* (`ReferenceComputerUse`) that talks to Reference app's
...
1. **Internal**: a singleton `cE` client (`main.js:48275-48495`) that the
   tool runtime calls.
2. **HTTP REST API**: Express routes `/api/computer-use/*` on Reference app's
...
The helper itself ships as an `.app` bundle inside Reference app's resources
directory (`Reference Computer Use.app/Contents/MacOS/ReferenceComputerUse`,
...
```
candidates = [
  resourcesPath + "Reference Computer Use.app/.../ReferenceComputerUse",
  __dirname + "../../computer-use/Reference Computer Use.app/.../ReferenceComputerUse",
  __dirname + "../../../computer-use/Reference Computer Use.app/.../ReferenceComputerUse",
  app.getAppPath() + "computer-use/.../ReferenceComputerUse",
  app.getAppPath() + "../computer-use/.../ReferenceComputerUse",
]
```
...
`main.js:48408` reads
`"Reference Computer Use helper not found. Run pnpm build:computer-use or reinstall Reference app."`
...
- `shutdown` gets a *short* 2 s timeout — Reference app is fine with a hard
  teardown if the helper doesn't ack.
- `show_cursor` is a near-universal arg: reference app can choose to display a
  visible cursor during automated input so the user sees what's
  happening.
- `ref` is reference app's opaque AX element handle — the model uses it after a
...
of dumping the full AX hierarchy, reference app flattens it to one short line per
...
Reference app's local Express server:
...
Two tables live in Reference app's main SQLite database. From `main.js:1946-1953`:
...
`(bundle_id, logged_at)` makes "show me everything reference app did to App X
...
1. **OpenAI Responses provider-executed path** (`main.js:11747-11764`).
   When the OpenAI Responses API returns a `computer_call`, reference app's
...
2. **Custom Browser tool family** (`main.js:28971-28973`, full list in
   `02-browser-tools.md`). These are Reference app-defined tools
...
schema — reference app uses the high-level Browser tools as the curated surface,
...
- Does reference app's helper handle macOS Sequoia's new "Screen & System Audio
  Recording" prompt? Need to test on real macOS 15.
- What's the latency budget for a single click? The 20 s timeout
  suggests it's not optimized for sub-second; reference app is tolerating long
  AX walks.
