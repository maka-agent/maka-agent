# 02 — Reference app output safety modes (compact / exact / passthrough)

> Source-grounded against `~/Downloads/reference-source/readable/main.js`.
> Pinned in round-2 [`02-send-response-flow-WIP.md`](../reference app-deep-dive-yuejing-round-2/02-send-response-flow-WIP.md)
> as a surface mentioned but not traced. This note traces the
> selection algorithm, the 8-profile budget table, the cascading
> truncation strategy, and the in-band `[reference app-output-safety: …]`
...
4. **`Jd` compact allowlist** (`main.js:17862-17875`): `Bash`,
   ...
   elsewhere. Cross-ref round-2 [`08-mcp-client.md`](../reference app-deep-dive-yuejing-round-2/08-mcp-client.md)
...
```js
Hd = /\[reference app-output-safety:\s*(exact|compact|passthrough)\s*\]/i
```
...
write `[reference app-output-safety: passthrough]` at its top, and the next
...
undefined)` deletes it). Open question: does reference app clear these
between threads?
...
4. Build truncation marker text:
   `\n...[reference app compacted N chars; <hint>]...\n`
...
drops below 0.35 (~35% of base limit per string). After that, reference app
...
if (("Bash" === e || "BashOutput" === e) && t && "object" == typeof t) {
  const e = n?.command;
  if (e && Wd(e) !== e) return t;  // command was MODIFIED (reference app rewrote it)
}
```

If reference app's command analyzer rewrote the user's Bash command
(e.g., wrapped with a timeout, added safety flags), the result
bypasses compaction. The reasoning: if reference app is going to introduce
...
2. **Adopt the in-band marker `[reference app-output-safety: …]`.** This
   is the killer feature: skills/prompts can declare per-tool
   safety mode without a config object. It also makes adopting
   reference app's skill ecosystem easier — reference app-format skills that use
...
5. **Outer shrink loop with floor.** The `0.8×` decay until
   `0.35×` floor is reference app's escape hatch for unexpectedly large
   responses. Without it, prompt overflow on a single 100k MCP
   response can wedge a turn.

## Open questions for round 3 of round-3

- Does reference app clear `Xd` (the per-tool runtime mode override) between
...
- Round 2: [`02-send-response-flow-WIP.md`](../reference app-deep-dive-yuejing-round-2/02-send-response-flow-WIP.md)
...
- Round 3: [`01-skills-system.md`](./01-skills-system.md) — skill
  bodies can emit the `[reference app-output-safety: …]` marker.
