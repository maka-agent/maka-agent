# 03 — Reference app MCP tool name collision logic (`bf` prefix + sanitizers)

> Source-grounded against `~/Downloads/reference-source/readable/main.js`.
> Pinned open question in round-2 [`08-mcp-client.md`](../reference app-deep-dive-yuejing-round-2/08-mcp-client.md):
...
```js
const s = vf(o, t);                  // parse description for [reference app-output-safety: …]
Qd(n, s.outputSafetyMode);            // register in Xd Map
```

`vf` at `main.js:25204-25211` reads two sources:
1. `tool.$.reference app/outputSafetyMode` — schema-level annotation from
   upstream MCP server.
2. In-band `[reference app-output-safety: …]` marker in `description`
...
3. **Detect sanitization collisions and warn.** reference app silently
...
4. **Forward MCP-supplied output safety mode.** The
   `$.reference app/outputSafetyMode` schema annotation + in-band marker
