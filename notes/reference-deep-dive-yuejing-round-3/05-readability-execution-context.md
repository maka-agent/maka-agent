...
> Pinned open question in round-2 [`10-chrome-relay.md`](../reference app-deep-dive-yuejing-round-2/10-chrome-relay.md)
> and [`11-browser-tools.md`](../reference app-deep-dive-yuejing-round-2/11-browser-tools.md):
> does reference app's Readability extractor run in the page context (via
...
logic, the reference app authors found enough noisy pages (cookie banners
...
After Readability returns HTML content, reference app converts it to
...
1. **DOM-less.** Server-side fetch + `cheerio` HTML parsing in
   ...
   loaded content fail.
2. **Page-context.** Same Electron BrowserWindow pattern reference app
   uses. JS runs, lazy loads complete, Readability extracts in-
   page. More expensive (BrowserWindow spawn) but works on
   modern web.

Reference app chose #2 across all 4 surfaces. The cost is one
...
1. **Page-context Readability, not main-process HTML parsing.**
   ...
   pages. The reference app pattern is well-engineered; reuse the same
...
4. **Two-tier fallback (semantic selectors with length floor →
   ...
   number per reference app's tuning.
...
- Round 2: [`10-chrome-relay.md`](../reference app-deep-dive-yuejing-round-2/10-chrome-relay.md)
  ...
  the four call sites.
- Round 2: [`11-browser-tools.md`](../reference app-deep-dive-yuejing-round-2/11-browser-tools.md)
