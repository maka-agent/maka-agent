...
describe('Command palette accessibility and visible copy', () => {
  ...
  it('uses COSS InputGroup for the palette input shell without restoring the old flex wrapper', async () => {
    const src = await readRepo('apps/desktop/src/renderer/command-palette.tsx');
    ...
    assert.match(
      ...
      'CommandPalette input shell must be COSS InputGroup with an accessible input label and trailing hint addon',
    );
    assert.doesNotMatch(
      src,
    ...
    assert.doesNotMatch(
      inputWrapStyle,
      /display:\s*flex/,
      'Palette input wrapper styling must not restore the old flex shell over COSS InputGroup',
    );
    assert.match(
      inputWrapStyle,
      /margin:\s*12px 14px;/,
