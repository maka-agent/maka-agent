...
describe('ExploreAgent read-only worker', () => {
  ...
  it('has a structured chat preview instead of raw JSON fallback', async () => {
    ...
    assert.match(previewBlock, /buildExploreAgentCopyPayloads/);
    assert.match(previewBlock, /copyPayloads/);
    assert.match(previewBlock, /summary: redactSecrets\(summaryText\)/);
    assert.match(previewBlock, /process: redactSecrets\(processText\)/);
    assert.match(previewBlock, /evidence: redactSecrets\(evidenceText\)/);
    assert.match(previewBlock, /candidate: redactSecrets\(candidateText\)/);
    assert.match(previewBlock, /matches: redactSecrets\(matchesText\)/);
    assert.match(previewBlock, /continuation: redactSecrets\(continuationText\)/);
    assert.match(previewBlock, /report: redactSecrets\(reportText\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('summary', copyPayloads\.summary\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('process', copyPayloads\.process\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('evidence', copyPayloads\.evidence\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('candidate', copyPayloads\.candidate\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('matches', copyPayloads\.matches\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('continuation', copyPayloads\.continuation\)/);
    assert.match(previewBlock, /copyFeedback\.copy\('report', copyPayloads\.report\)/);
