...
describe('terminal truncation handoff contract', () => {
  it('shows a deep-research handoff when terminal output is capped', async () => {
    ...
    assert.match(
      ...
      'The capped-output handoff copy action should use the vendored COSS Button primitive.',
    );
    assert.match(
      src,
      /<CossButton[\s\S]*variant="ghost"[\s\S]*size="sm"[\s\S]*className="maka-tool-terminal-copy"/,
      'The capped-output handoff copy action should keep its ghost/sm affordance through COSS Button props and a semantic class hook.',
    );
    assert.doesNotMatch(
      src,
    ...
    assert.doesNotMatch(
      ...
      'The capped-output handoff copy action should rely on COSS Button size props instead of redundant data-size styling.',
    );
    assert.match(
      src,
