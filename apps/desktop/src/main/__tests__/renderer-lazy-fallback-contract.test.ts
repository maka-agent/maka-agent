...
describe('renderer lazy fallback contract', () => {
  ...
  it('keeps module lazy chunks on compact non-null fallbacks', async () => {
    ...
    assert.match(modulePages, /props\.embedded\s*\? <ModulePanelFallback message=\{copy\.loadingSkills\} \/>\s*:\s*<ModulePageFallback label=\{copy\.skills\} message=\{copy\.loadingSkills\} \/>/);
