...
describe('Settings coming-soon cleanup contract', () => {
  ...
  it('keeps planned bot platforms out of the credentials-readiness flow', async () => {
    ...
    assert.doesNotMatch(settings, /机器人运行时尚未接入|代码中还没有这个平台的运行时|平台运行时尚未接入|运行时未开放|开放前|收发 smoke/, 'planned bot copy must not expose implementation-status placeholder language');
