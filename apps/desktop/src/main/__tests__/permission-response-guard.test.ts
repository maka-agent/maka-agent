...
describe('permission prompt response guard', () => {
  it('keeps allow/deny decisions single-flight for a request id', async () => {
    ...
    assert.match(prompt, /responsePending\s*\?\s*'正在提交…'/);
    assert.match(
      submit,
      /props\.request\.kind === 'additional_permissions' \|\| props\.request\.kind === 'sandbox_escalation'[\s\S]*\? \{\}[\s\S]*: \{ rememberForTurn:/,
      'one-call sandbox grants must never be remembered for the turn',
    );
  });
