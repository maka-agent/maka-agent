...
describe('chat tool-card migration contract (#332 PR3b)', () => {
  ...
  it('keeps the computed-style fixture covering every production tool status', async () => {
    ...
    // truth source is the tool-activity result projection's `STATUS_LABEL` (a
    // `Record` over the full `ToolActivityItem['status']` union — its keys ARE
    // every production status; the record lives in
    // tool-activity/result-projection.ts since the render file holds only
    // components). The harness must render a card for each: a new status that
    ...
    assert.ok(labelStart !== -1, 'failed to locate the tool STATUS_LABEL declaration in tool-activity/result-projection.ts');
    ...
    assert.ok(statuses.length >= 5, 'failed to parse STATUS_LABEL keys from tool-activity/result-projection.ts');
