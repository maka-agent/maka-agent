...
describe('home composer new-chat model picker', () => {
  ...
  it('does not let stale new-chat send creation steal the active session after navigation', async () => {
    ...
    assert.match(
      sendBlock,
      /if \(newChatOwner && isNewChatSendSurfaceActive\(newChatOwner\)\) \{[\s\S]*setNavSelection\(\{ section: 'sessions', filter: 'chats' \}\);[\s\S]*setActiveId\(session\.id\);[\s\S]*showOptimisticUserMessage\(session\.id, turnId, text, attachments, \{ replaceCurrentMessages: true \}\);[\s\S]*\}/,
