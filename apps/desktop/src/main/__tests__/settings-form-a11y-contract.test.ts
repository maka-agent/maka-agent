...
describe('Settings form accessibility labels', () => {
  it('keeps Settings secondary surfaces close to reference implementation card geometry', async () => {
    ...
    assert.match(providerCatalogBadge, /border-radius:\s*var\(--radius-control\);/, 'Provider catalog badges (category / preview / login) ride the capsule control tier (MOBBIN-RESTYLE-0)');
    assert.match(modelTableChip, /border-radius:\s*var\(--radius-control\);/, 'Settings model capability chips ride the capsule control tier (MOBBIN-RESTYLE-0)');
    assert.match(modelTableDefaultBadge, /border-radius:\s*var\(--radius-control\);/, 'Settings model default badge ride the capsule control tier (MOBBIN-RESTYLE-0)');
    assert.match(connectionBadge, /rounded-\[var\(--radius-control\)\]/, 'Settings status badges (Chip primitive) ride the capsule control tier (MOBBIN-RESTYLE-0)');
    assert.match(settingsBadge, /rounded-\[var\(--radius-control\)\]/, 'Generic Settings badges (Chip primitive) ride the capsule control tier (MOBBIN-RESTYLE-0)');
    assert.doesNotMatch(providerCatalogBadge, /border-radius:\s*var\(--radius-pill\);/, 'Provider catalog badges must reference the control tier token, not raw --radius-pill');
    assert.doesNotMatch(modelTableChip, /border-radius:\s*var\(--radius-pill\);/, 'Settings model capability chips must reference the control tier token, not raw --radius-pill');
    assert.doesNotMatch(modelTableDefaultBadge, /border-radius:\s*var\(--radius-pill\);/, 'Settings model default badge must reference the control tier token, not raw --radius-pill');
    assert.doesNotMatch(connectionBadge, /rounded-\[var\(--radius-pill\)\]/, 'Settings connection badges (Chip primitive) must reference the control tier token, not raw --radius-pill');
    assert.doesNotMatch(settingsBadge, /rounded-\[var\(--radius-pill\)\]/, 'Generic Settings badges (Chip primitive) must reference the control tier token, not raw --radius-pill');
