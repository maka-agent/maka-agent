...
Token authoring rule: custom CSS variables go in `maka-tokens.css`. The `@theme` Tailwind bridge is split — most aliases (color / typography / spacing / radius) live in `maka-tokens.css`, a few values (e.g. `--shadow-maka-panel`) live in `styles.css`; both locations are contract-pinned, so check which file owns a given bridge value before moving it. New component-local vars should carry `/* local: ... */` (existing ones don't all have it yet). No new hardcoded color / radius / z-index.
