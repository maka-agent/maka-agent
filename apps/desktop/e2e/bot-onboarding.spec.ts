...
test('关闭扫码弹窗会取消迟到结果，过期二维码可以重新生成', async ({ botSettingsWindow: page }) => {
  ...
  await settings.getByRole('button', { name: '扫码接入' }).click();
  const wechatDialog = page.getByRole('dialog', { name: '连接微信扫码接入' });
  await expect(wechatDialog.getByRole('img', { name: '连接微信二维码' })).toBeVisible();
  ...
  await settings.getByRole('button', { name: '开始快捷接入' }).click();
