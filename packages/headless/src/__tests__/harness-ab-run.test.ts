...
describe('runHarnessAbComparison', () => {
  ...
  test('explicitly backfills only unscored harness cells', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'maka-harness-ab-backfill-'));
    try {
      const promptPath = join(dir, 'empty-system-prompt.txt');
      const resultsPath = join(dir, 'results.jsonl');
      await writeFile(promptPath, '', 'utf8');
      ...
      maka.harborRunner = async (input) => {
        if (!initialRun) return successfulMakaRunner(input);
        if (input.task.id === 'b') {
          calls.push('b:maka');
          throw new HarborInfraError('container failed after launch');
        }
        const output = await successfulMakaRunner(input);
        if (input.task.id !== 'c') return output;
        return {
          ...output,
          harbor: { reward: 0 },
          cell: { ...output.cell, status: 'failed', errorClass: 'timeout' },
        };
      };
      const input = {
        runId: 'glm-harness-ab',
      ...
      const first = await runHarnessAbComparison(input);
      assert.equal(first.pairedAttempts.evaluatedPairs, 1);
      assert.equal(calls.length, 6);

      initialRun = false;
      const resumed = await runHarnessAbComparison(input);
      assert.equal(resumed.pairedAttempts.evaluatedPairs, 1);
      assert.equal(calls.length, 6);

      const backfilled = await runHarnessAbComparison({
        ...input,
        backfillUnscoredCells: true,
      });

      assert.equal(backfilled.pairedAttempts.evaluatedPairs, 3);
      assert.equal(calls.filter((call) => call === 'a:maka').length, 1);
      assert.equal(calls.filter((call) => call === 'a:opencode').length, 1);
      assert.equal(calls.filter((call) => call === 'b:maka').length, 2);
      assert.equal(calls.filter((call) => call === 'b:opencode').length, 1);
      assert.equal(calls.filter((call) => call === 'c:maka').length, 2);
