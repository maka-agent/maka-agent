...
export function createTaskRunReader(storageRoot: string): TaskRunReader {
  return taskRunReaderFacade(new FileTaskRunStore(storageRoot), (operation) => operation());
}
...
class FileTaskRunStore implements TaskRunWriter {
  ...
  async appendEvent(taskRunId: string, event: TaskEvent): Promise<void> {
    ...
    const previous = this.queues.get(locator) ?? Promise.resolve();
    const next = previous.then(() => this.appendTaskRunEvent(locator, taskRunId, event));
    this.queues.set(
      locator,
      next.catch(() => undefined),
    );
