...
For a live run, remove `MAKA_HARNESS_AB_DRY_RUN` and set `MAKA_HARNESS_AB_KEY_FILE` to a credential file outside git. Resume with the same output directory and run id; changing `MAKA_HARNESS_AB_LIMIT` from `40` to `89` runs only missing cells. The immutable manifest rejects other configuration changes.
