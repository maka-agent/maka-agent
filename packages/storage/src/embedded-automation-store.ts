...
/** Minimal constraint for records stored by the embedded Automation owner. */
...
/** Production Desktop/CLI store until the M5 owner cutover. */
...
class FileAutomationStore<T extends AutomationRecord> implements AutomationStore<T> {
  ...
  async save(automation: T): Promise<void> {
    await chainWrite(this.writeQueue, FileAutomationStore.QUEUE_KEY, async () => {
      ...
      const index = current.findIndex((candidate) => candidate.id === automation.id);
      if (index >= 0) current[index] = automation;
      else current.push(automation);
  ...
  async remove(id: string): Promise<void> {
    await chainWrite(this.writeQueue, FileAutomationStore.QUEUE_KEY, async () => {
      const current = await this.loadAll();
      const filtered = current.filter((automation) => automation.id !== id);
...
function isAutomationFile(value: unknown): value is AutomationFile {
  ...
  return item.version === 1 && Array.isArray(item.automations);
}
