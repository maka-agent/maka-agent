...
test('creates and atomically updates a Claude-compatible mcp.json', async () => {
  ...
  const next = await store.upsert('filesystem', {
    command: 'npx', args: ['-y', 'server'], env: { TOKEN: 'secret' }, enabled: true,
  });
  assert.equal(next.mcpServers.filesystem && 'command' in next.mcpServers.filesystem
    ? next.mcpServers.filesystem.command : undefined, 'npx');
  assert.deepEqual(JSON.parse(await readFile(join(root, 'mcp.json'), 'utf8')), next);
  if (process.platform !== 'win32') assert.equal((await stat(join(root, 'mcp.json'))).mode & 0o777, 0o600);
...
test('serializes concurrent updates without corrupting the file', async () => {
  const root = await tempRoot();
  const store = createMcpConfigStore(root);
  await Promise.all(Array.from({ length: 20 }, (_, index) => store.upsert(`s-${index}`, { command: `cmd-${index}` })));
...
test('rejects corrupt files and unsafe or invalid configs', async () => {
  ...
  assert.throws(() => normalizeMcpConfig({ version: 1, mcpServers: { constructor: { command: 'x' } } }), /Invalid server id/u);
  assert.throws(() => normalizeMcpConfig({ version: 1, mcpServers: { bad: { url: 'file:///tmp/x' } } }), /http or https/u);
  assert.throws(() => normalizeMcpConfig({ version: 1, mcpServers: { bad: { url: 'https://user:secret@example.com/mcp' } } }), /embedded credentials/u);
