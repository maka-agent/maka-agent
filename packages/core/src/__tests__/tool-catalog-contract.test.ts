...
describe('tool catalog contract', () => {
  ...
  it('requires every surface member to be a catalog tool row', () => {
    for (const surface of MAKA_CATALOG_SURFACES) {
      ...
      for (const name of surface.toolNames) {
        assert.ok(
          catalogToolByName(name),
          `surface ${surface.id} references unknown tool ${name}`,
        );
      }
    }
  });
  ...
  it('covers expected host fixture names as catalog subsets', () => {
    ...
    const fixtures: Record<ToolHostId, readonly string[]> = {
      desktop: [
        'Bash', 'Read', 'Write', 'Edit', 'Glob', 'Grep', 'FormatJson',
        'StopBackgroundTask', 'WriteStdin',
        'AskUserQuestion', 'Skill', 'WebSearch', 'ExploreAgent', 'Automation',
        'GoalSet', 'GoalClear', 'GoalStatus', 'GoalPause', 'GoalResume',
        'task_create', 'task_update', 'task_list', 'task_get',
        'OfficeDocument', 'OfficeDocumentEdit',
        'browser_navigate', 'browser_snapshot', 'browser_click', 'browser_type', 'browser_wait', 'browser_extract',
        'maka_computer', 'RiveWorkflow',
        'agent_spawn', 'agent_swarm', 'agent_list', 'agent_output',
        'team_message', 'team_inbox', 'team_task_list', 'team_task_claim',
      ],
      cli: [
        'Bash', 'Read', 'Write', 'Edit', 'Glob', 'Grep', 'FormatJson',
        'StopBackgroundTask', 'WriteStdin',
        'AskUserQuestion', 'Skill', 'Automation',
        'GoalSet', 'GoalClear', 'GoalStatus', 'GoalPause', 'GoalResume',
        'agent_spawn', 'agent_swarm', 'agent_list', 'agent_output',
      ],
      headless: [
