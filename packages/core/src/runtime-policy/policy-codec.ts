...
export function normalizeRuntimePolicySection<K extends keyof RuntimePolicy>(
  section: K,
  value: unknown,
): RuntimePolicy[K] {
  switch (section) {
    case 'networkProxy':
      return normalizeNetworkProxy(value) as RuntimePolicy[K];
    case 'personalization':
      return normalizePersonalization(value) as RuntimePolicy[K];
    case 'memory':
      return normalizeMemory(value) as RuntimePolicy[K];
    case 'workspaceInstructions':
      return normalizeWorkspaceInstructions(value) as RuntimePolicy[K];
    case 'privacy':
      return normalizePrivacy(value) as RuntimePolicy[K];
    case 'chatDefaults':
      return normalizeChatDefaults(value) as RuntimePolicy[K];
    case 'webSearch':
      return normalizeWebSearch(value) as RuntimePolicy[K];
  }
}
