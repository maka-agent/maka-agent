 ...
 * its own session, so it must only fire when that session is genuinely idle —
 * not mid-turn ('running'), waiting on a human ('waiting_for_user'), under
 * review, blocked, aborted, or archived. Shared by every host so the desktop
 * and CLI gates cannot diverge.
 */
export const HEARTBEAT_IDLE_STATUSES: ReadonlySet<string> = new Set(['active', 'done']);
