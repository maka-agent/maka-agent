/**
 * Goal evaluator — CC-style external judge. Uses a cheap/fast model (e.g. haiku)
 * to decide, after each turn, whether the goal is met, impossible, making
 * progress, or waiting on an external event.
 *
...
export interface GoalEvaluation {
  ...
  waiting: boolean;
  /** Suggested seconds to wait before re-checking, when `waiting`. */
  waitSeconds?: number;
  /**
...
export interface GoalEvaluatorDeps {
  /**
   * Single-shot LLM call for goal evaluation. Should use a cheap/fast model.
   * The evaluator must not run tools or read files — it judges from text only.
   */
...
{"met": boolean, "impossible": boolean, "progress": boolean, "waiting": boolean, "wait_seconds": number, "reason": "one sentence"}
...
- waiting: true if the agent is correctly blocked on an external event it cannot speed up (CI run, deploy, remote queue, human review). Set wait_seconds to a sensible poll interval (default 60).
...
export function parseGoalEvaluation(raw: string): GoalEvaluation {
  ...
  try {
    ...
    const waitSecondsRaw = parsed.wait_seconds;
    const waitSeconds = typeof waitSecondsRaw === 'number' && Number.isFinite(waitSecondsRaw) && waitSecondsRaw > 0
      ? Math.min(3600, Math.round(waitSecondsRaw))
      : undefined;
    return {
...
export async function evaluateGoal(
  ...
  try {
    const result = await Promise.race([deps.evaluate(prompt), timeout]);
