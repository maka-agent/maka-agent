...
describe('parseGoalEvaluation', () => {
  ...
  test('parses waiting with wait_seconds', () => {
    const r = parseGoalEvaluation('{"met": false, "impossible": false, "progress": false, "waiting": true, "wait_seconds": 120, "reason": "CI running"}');
    assert.equal(r.waiting, true);
    assert.equal(r.waitSeconds, 120);
  });

  test('clamps wait_seconds to <= 3600', () => {
    const r = parseGoalEvaluation('{"met": false, "waiting": true, "wait_seconds": 999999, "reason": "x"}');
    assert.equal(r.waitSeconds, 3600);
  });

  test('drops non-positive wait_seconds', () => {
    const r = parseGoalEvaluation('{"met": false, "waiting": true, "wait_seconds": 0, "reason": "x"}');
