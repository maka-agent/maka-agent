...
const declaration = { mode: 'require_escalated', justification: 'Write the exact requested output.' } as const;
...
describe('ToolRuntime sandbox escalation orchestration', () => {
  test('auto-reviews execute mode, emits no interactive request, and exposes one exact grant', async () => {
    ...
    const tool = bashTool((_args, context) => {
      receivedContext = context.permissionContext;
      return { ok: true };
    });

    const result = await h.runtime.wrapToolExecute(tool, 'turn-1', {
  ...
  test('auto-review denial never invokes the command implementation', async () => {
    let called = false;
    let reviews = 0;
    const h = harness({
      review: async () => {
    ...
    assert.deepEqual(result, { error: '自动审批已拒绝权限请求：User intent is ambiguous.' });
  ...
  test('blocks an identical escalation retry in the same turn after auto-review failure', async () => {
    let reviews = 0;
    let executions = 0;
    const h = harness({
      review: async () => {
        ...
        return { outcome: 'allow', riskLevel: 'high', rationale: 'Would allow a retry.' };
      },
    });
    const tool = bashTool(() => {
      executions += 1;
    ...
    const execute = h.runtime.wrapToolExecute(tool, 'turn-1', {
      push: (event) => h.events.push(event),
    });
    ...
    const nextTurn = await h.runtime.wrapToolExecute(tool, 'turn-2', {
      push: (event) => h.events.push(event),
    })(args, {
      toolCallId: 'tool-3', abortSignal: new AbortController().signal,
    });
    assert.deepEqual(nextTurn, { ok: true });
...
function bashTool(impl: MakaTool['impl']): MakaTool {
  return {
    ...
    parameters: z.object({
      command: z.string(),
      sandbox_permissions: z.object({ mode: z.literal('require_escalated'), justification: z.string() }),
    }),
...
function harness(autoReviewer: AutoApprovalReviewer) {
  ...
  const header: SessionHeader = {
    ...
    permissionMode: 'execute', schemaVersion: 1,
  };
