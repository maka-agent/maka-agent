...
async function main(): Promise<void> {
  ...
  const response = await executeFilesystemWorkerRequest(parsed.data, {
...
class RequestTooLargeError extends Error {
  constructor() {
    super('Filesystem worker request exceeded the size limit.');
    this.name = 'RequestTooLargeError';
  }
}

function writeResponse(response: FilesystemWorkerResponse): void {
  const validated = FilesystemWorkerResponseSchema.parse(response);
