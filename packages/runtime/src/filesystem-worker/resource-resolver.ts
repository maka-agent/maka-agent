...
export type FilesystemWorkerResourceLocation =
  ...
  | { kind: 'desktop-development'; moduleUrl?: string }
...
export type FilesystemWorkerResourceResult =
  ...
  | {
      ok: false;
      reason: 'bundle_not_found' | 'bundle_invalid';
      path: string;
    };
...
export async function resolveFilesystemWorkerBundle(
  ...
  try {
    ...
    const reason = nodeErrorCode(error) === 'ENOENT' ? 'bundle_not_found' : 'bundle_invalid';
    return { ok: false, reason, path: candidate };
  }
}
