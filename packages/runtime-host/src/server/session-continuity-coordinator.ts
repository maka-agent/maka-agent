...
export class SessionContinuityCoordinator {
	readonly handlers: SessionContinuityOperationHandlerMap = {
		...
		"subscription.close": async (input, context) => {
			const closed = this.#closeSubscription(
	...
	readonly #sessionLaneTails = new Map<string, Promise<void>>();
	#closed = false;
	...
	async refreshCanonical(sessionId: string): Promise<void> {
		await this.#runInSessionLane(sessionId, async () => {
			if (this.#closed) return;
			const state = this.#sessions.get(sessionId);
			if (
				!state ||
				(state.subscribers.size === 0 && !state.terminalPublicationFence)
			)
				return;
	...
	async holdTerminalPublication(
		...
		await this.#runInSessionLane(sessionId, async () => {
			...
			if (existing) {
				...
				throw new Error(
					"Session already has a different terminal publication fence",
				);
			}
	...
	async publishTerminalProjection(
		...
		await this.#runInSessionLane(sessionId, async () => {
			...
			if (
				...
				throw new Error(
					"Terminal publication does not own the Session continuity fence",
				);
			}
			...
			if (!canonical) {
				throw new Error(
					"Canonical Session projection is not terminal for the fenced Turn",
				);
			}
		...
		try {
			return await this.#runInSessionLane(sessionId, async () => {
				if (this.#connections.get(connectionId) !== connection) {
					throw new Error(
						"Runtime Host connection closed during subscription open",
					);
				}
				const canonical = await this.#readCanonicalProjection(sessionId);
				if (this.#connections.get(connectionId) !== connection) {
	...
	#activate(connectionId: string, subscriptionId: string): void {
		...
		if (!subscriber || subscriber.activated || subscriber.phase === "closed")
			return;
	...
	#evictSlowSubscriber(subscriber: Subscriber): void {
		...
		for (const character of event.text) {
			...
			if (
				characterRawBytes > SESSION_LIVE_DELTA_MAX_BYTES ||
				characterWireBytes > wireTextLimit
			) {
				throw new Error(
					"Session delta character exceeds the wire frame budget",
				);
			}
			chunk += character;
	...
	#removeSubscriber(subscriber: Subscriber): void {
		...
		if (state && removed && state.subscribers.size === 0) {
	...
	#scheduleInactiveStateCleanup(
		sessionId: string,
		state: SessionProjectionState,
	): void {
		void this.#runInSessionLane(sessionId, () => {
			if (
	...
	#runInSessionLane<T>(
		...
		const previous = this.#sessionLaneTails.get(sessionId) ?? Promise.resolve();
		const current = previous.catch(() => undefined).then(operation);
		const tail = current.then(
			() => undefined,
			() => undefined,
		);
		this.#sessionLaneTails.set(sessionId, tail);
		void tail.finally(() => {
			if (this.#sessionLaneTails.get(sessionId) === tail) {
				this.#sessionLaneTails.delete(sessionId);
			}
		});
		return current;
	}
...
function requirePublicationFenceIdentity(
	...
	if (
		...
		throw new Error(
			"Canonical Session projection identity does not match its publication fence",
		);
	}
	return rootTurn;
}
