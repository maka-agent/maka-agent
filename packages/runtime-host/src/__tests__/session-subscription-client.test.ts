...
} from "../transport/framed-transport.js";
import {
		...
		async (transport, hostEpoch) => {
			...
			await transport.writeEncoded(
				Buffer.concat([
					...
					encodeProtocolFrame(
						deltaFrame(hostEpoch, reloaded.subscriptionId, 1),
					),
				]),
			);
...
test("fails the connection when a live subscription frame changes Host Epoch", async () => {
	await withProtocolPeer(
		async (transport, hostEpoch) => {
			...
			await transport.write(
				deltaFrame("different-host-epoch", opened.subscriptionId, 1),
			);
			await transport.closed;
		},
		async (connection) => {
			...
			await assert.rejects(
				...
				(error: unknown) =>
					error instanceof RuntimeHostSubscriptionError &&
					error.reason === "host_epoch_changed",
			);
...
test("fails the connection when an internal subscription close is rejected", async () => {
	await withProtocolPeer(
		...
		async (connection) => {
			...
			assert.equal(
				(await subscription[Symbol.asyncIterator]().next()).done,
				false,
			);
			await assert.rejects(
				() => subscription[Symbol.asyncIterator]().next(),
			...
			await assert.rejects(
				...
				(error: unknown) =>
					...
					error.operation === "subscription.close",
			);
		},
	);
});
...
test("drains a coalesced mixed burst beyond the transport queue frame bound", async () => {
	await withProtocolPeer(
		...
		async (connection) => {
			...
			const statuses = Array.from({ length: MIXED_BURST_REQUESTS }, () =>
				connection.status(5_000),
			);
			const first = await subscription[Symbol.asyncIterator]().next();
...
async function withProtocolPeer(
	...
	const server = createServer((socket) => {
		void serve(new FramedTransport(socket), hostEpoch).then(
			serverTask.resolve,
			serverTask.reject,
		);
	});
	try {
		...
		await removeHostRegistration(controlDirectory, hostEpoch).catch(
			() => undefined,
		);
