...
test("a connection accepted during recovery resolves ready handlers and continuity without reconnecting", async () => {
	...
	try {
		...
		const queuedDelta = continuityService.acceptAssistantDelta(
			"session",
			"run-turn",
			{
				type: "text_delta",
		...
		if (
			!("kind" in projection) ||
			projection.kind !== "subscription.session_projection"
		) {
		...
		if (
			!("kind" in concurrentMarker) ||
			concurrentMarker.kind !== "subscription.session_delta"
		) {
		...
		await closeSubscription(
			transport,
			concurrentOpen.subscriptionId,
			"close-concurrent-open",
		);
		await continuityService.refreshCanonical("session");
		const reopenedSession = await openSubscription(
			transport,
			"session",
			"reopen-session",
		);
		...
		await closeSubscription(
			transport,
			reopenedSession.subscriptionId,
	...
	await withContinuityHost(
		...
		async ({ continuity, endpoint }) => {
			...
			try {
				...
				const opening = openSubscription(
					opener,
					"session",
					"disconnecting-open",
				).then(
...
test("a terminal publication fence withholds terminal projection until explicit publish", async () => {
	...
	await withContinuityHost(
		"session",
		async ({ continuity, endpoint }) => {
			await continuity.holdTerminalPublication(
				"session",
				"turn",
				"run-turn",
			);
			const subscriber = await openAcceptedTransport(endpoint, "subscriber");
			const terminalOpener = await openAcceptedTransport(
				endpoint,
				"terminal-opener",
			);
			try {
				...
				await continuity.publishTerminalProjection(
					"session",
					"turn",
					"run-turn",
					() => undefined,
				...
				if (
					!("kind" in terminal) ||
					terminal.kind !== "subscription.session_projection"
				) {
				...
				const afterPublish = await openAcceptedTransport(
					endpoint,
					"after-publish",
				);
				try {
	...
	await withContinuityHost(sessionId, async ({ continuity, endpoint }) => {
		...
		try {
			...
			while (received.length < text.length) {
				...
				assert.ok(
					Buffer.byteLength(frame.delta.text, "utf8") <=
						SESSION_LIVE_DELTA_MAX_BYTES,
				);
				assert.ok(
					encodeProtocolFrame(frame).byteLength <= RUNTIME_HOST_MAX_FRAME_BYTES,
				);
	...
	return new SessionContinuityCoordinator(
		...
		async (requestedSessionId) =>
			requestedSessionId === sessionId
				? canonicalProjection(sessionId, 1)
				: null,
	);
}
