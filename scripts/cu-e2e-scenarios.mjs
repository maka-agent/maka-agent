...
export const CU_E2E_SCENARIOS = Object.freeze([
  ...
  {
    id: 'l1-background-zoom',
    level: 'L1',
    prompt:
      'Observe "Maka CUA L1 Background Zoom Target" twice so its AX tree is stable, then use zoom with the second observation on region [80,80,360,280]. The separate "Maka CUA L1 Zoom Occluder" intentionally covers the target. Do not move, close, activate, or click either window.',
    fixtureSetup: {
      layout: 'overlap',
      windows: [
        {
          id: 'target',
          title: 'Maka CUA L1 Background Zoom Target',
          kind: 'observe',
          verificationCode: 'CUA-ZOOM-417',
        },
        {
          id: 'occluder',
          title: 'Maka CUA L1 Zoom Occluder',
          kind: 'occluder',
        },
      ],
    ...
    expectedState: [
      { windowId: 'target', path: 'verificationCode', equals: 'CUA-ZOOM-417' },
      { windowId: 'target', path: 'interactions', equals: 0 },
      { windowId: 'occluder', path: 'interactions', equals: 0 },
    ],
    forbiddenEffects: [
      invariant('target', 'interactions', 0, 'background zoom must not mutate the target'),
      invariant('occluder', 'interactions', 0, 'background zoom must not touch the occluder'),
    ],
    allowedActions: ['observe', 'zoom'],
    minimumActionCounts: { observe: 2, zoom: 1 },
    maxTotalActions: 3,
    maxActionCounts: { observe: 2, zoom: 1 },
    expectedActionSequence: ['observe', 'observe', 'zoom'],
    contractChecks: [
      'observation-window-frame-binding',
      'zoom-crop-coordinate-space',
      'two-window-isolation',
  ...
  {
    id: 'l2-background-semantic',
    level: 'L2',
    prompt:
      'Operate "Maka CUA L2 Background Semantic Target" while the separate "Maka CUA L2 Semantic Occluder" remains above it. Observe the target twice so its AX tree is stable, set "Background text" to "background-cua" with set_value from the second observation, use the fresh observation to press Space on "Background checkbox" with press_key, then use that fresh observation to click "Commit background" exactly once. Never move, activate, close, or interact with the occluder. Do not use coordinates.',
    fixtureSetup: {
      layout: 'overlap',
      windows: [
        {
          id: 'target',
          title: 'Maka CUA L2 Background Semantic Target',
          kind: 'background-semantic',
        },
        {
          id: 'occluder',
          title: 'Maka CUA L2 Semantic Occluder',
          kind: 'occluder',
        },
      ],
      zOrder: ['target', 'occluder'],
    ...
    expectedState: [
      { windowId: 'target', path: 'text', equals: 'background-cua' },
      { windowId: 'target', path: 'checked', equals: true },
      { windowId: 'target', path: 'buttonClicks', equals: 1 },
      { windowId: 'occluder', path: 'interactions', equals: 0 },
    ],
    forbiddenEffects: [
      invariant('target', 'buttonOverClicks', 0, 'the background button must not be clicked twice'),
      invariant('occluder', 'interactions', 0, 'the foreground occluder must remain untouched'),
    ],
    allowedActions: ['observe', 'set_value', 'press_key', 'click_element'],
    minimumActionCounts: { observe: 2, set_value: 1, press_key: 1, click_element: 1 },
    maxTotalActions: 8,
    maxActionCounts: { observe: 4, set_value: 1, press_key: 1, click_element: 1 },
    contractChecks: [
      'observation-window-frame-binding',
      'fresh-post-action-observation',
      'semantic-action-coverage',
