...
| `aborted` | 用户 cancel 或 turn-level cancel（侧栏隐藏） | n/a — sidebar 不显示 | (terminal) |
...
**Sidebar grouping**（PR109b, locked order，跟 `aborted` 隐藏）：

```
Pinned (synthetic, only when `pinFirst` + any flagged session)
  ↓
Running → Waiting → Blocked → Active → Review → Done → Archived (collapsed default)
```

`pinned` 抽出 flagged session 但不在某个 status 群里重复出现（去重）。
`archived` 是唯一 collapsible 的 group（写 localStorage 不持久，每次 session
重置，跟「dormant by default」语义一致）。
...
- `aborted` 在 sidebar 隐藏；将来加 navSelection filter `aborted` 再露出
