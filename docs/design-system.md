...
### 9.1 ArtifactRecord shape — @kenji audit item 2

未来 Artifact 工作区（Files / Terminal / Preview）需要的核心数据 record。
当前 alma 在 `WorkspaceDiff` + snapshot.* preload 8 个方法暴露；Maka 侧需要决
定：

- per-session vs per-workspace 工作目录
- snapshot 版本号是 monotonic int 还是 git-style SHA
- 删除时是软删还是硬删
