...
- 不是 reference app re-engineering doc 的复述，详细对照见
  `notes/maka-vs-reference app-gap-v2.md`。
...
下列 surface 是 @kenji v2 audit 8 系统 + @yuejing reference app 差距分析提到、但尚未实装的。每条必须按 §11 release-gate mapping 同时声明 fixture scenario / screenshots / failure state / smoke path / test gate。
...
| **Quick Chat panel** (global hotkey BrowserWindow) | yuejing reference app 差距分析 | focus = composer auto-focus on open；loading = AX 抓取进度；error = 权限缺失 / hotkey 冲突 inline；empty = 首次使用提示 | §9.7 (扩) |
...
Quick Chat（参考 reference app macOS panel + global shortcut）尚未引入；引入时
...
无 automation；reference app 把这三类做成可审计 + 可禁用的一等公民。
...
来自 @kenji 自己的 `notes/maka-reference app-reference app-design-gap-v2.md` +
