...
- Phase 1 的 safe-boundary continuation 已实现，并由 `MAKA_RUNTIME_SAFE_BOUNDARY_RESUME=1` 控制自动启动；
- Phase 2 的 SQLite schema、Tool Journal、T1/T2、operationId、CAS、partial snapshot、JSONL 幂等导入/显式导出及 CLI/Desktop 单一 canonical writer 接线已实现；
- Phase 2 的生产切换默认关闭，仅当 `MAKA_RUNTIME_SQLITE_CANONICAL=1` 时，RuntimeEvent 与 Tool Journal 才以同一个 SQLite store 为唯一事实源；关闭时保持原 JSONL 行为，系统不会双写两个 RuntimeEvent 真相源；
...
1. **Intent before effect**：真实 tool implementation 只能在 function-call fact 与 prepared fact 原子提交后开始。
...
- prepared 表示 intent 已提交，不表示副作用未发生；
...
可以维护 `tool_operations` 快速状态表，但它只是 projection。删除后必须能从 journal 重建。
...
`tool_operations` 是查询投影；`tool_journal_events` 与 `runtime_events` 是恢复事实。并行 tool calls 的数据库 commit order 定义 durable event_seq，不再依赖毫秒时间戳解决因果顺序。
...
2. 分配 event_seq；
3. append RuntimeEvent(function_call)；
4. append ToolJournal(prepared)；
5. 更新 tool_operations projection；
6. commit。

只有 T1 成功后：

- UI 才接收 durable-linked tool_start；
...
不能假设所有平台上的 `fs.renameSync` 都有相同 replace 语义。Windows、macOS、Linux 必须分别做真实进程 crash test，并为不支持的场景提供受控 fallback。
...
禁止生产路径让 SQLite 与 JSONL 任一写成功都被视为 committed。正确方式是：
...
4. 使用原 event ID 幂等 INSERT；
5. 按原 ledger 顺序分配 event_seq；
6. 记录 source file size/hash 与 migration high water；
...
当前状态：已实现，CLI/Desktop 通过 `MAKA_RUNTIME_SQLITE_CANONICAL=1` 显式启用；默认仍使用 JSONL。启用时先幂等导入旧 RuntimeEvent JSONL，再将 SQLite 同时作为 `RuntimeEventStore` 与 `RuntimeCommitSink`，正常执行不再写 RuntimeEvent JSONL。Session message JSONL 与 AgentRun operational JSONL 仍是兼容投影/运行记录，不构成第二个 RuntimeEvent source of truth。
