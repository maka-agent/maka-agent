...
| `semanticCompact` | Active-loop messages plus available Turn RuntimeEvent refs | During `prepareStep` | `SemanticCompactBlock` is a best-effort diagnostic record; accepted messages live in the current backend projection | Adjacent, not equivalent to V2 checkpoint |

They share the direction “do not mutate canonical source,” but their guarantees are not identical.

Current `semanticCompact`, in particular, asks an LLM to summarize an active provider-message span, validates coverage and savings, and reuses an accepted projection in later AI SDK steps. Its source index may rely on invocation-local `ModelMessage`, however, and failure of the block recorder does not roll back an already accepted provider projection. It is not the same protocol as V2 history checkpointing, where durable append succeeds before replacement replay.
