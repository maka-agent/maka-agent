...
没有 filesystem worker 的 host 可以在启动装配时**显式选择** `host_local` data plane，
以保留 Linux/有限支持平台上的 native 文件事务能力；该选择必须记录
`prepared_file_mutation_execution_owner=host_local`，且不能在 worker 请求失败后动态切换。
也就是说，`host_local` 是可观察的部署 capability，不是错误路径里的 fallback。
worker error 不能只按 `reason` 字符串分类：启动前已取消能够证明 effect 未开始，走普通取消；
进程启动后的 abort、timeout、crash、response overflow 或协议响应不可解析，都只能标记为
`effectMayHaveStarted` 并保持 unsettled。若 source Run 已是 `cancelled`，authoritative planner
对未收敛 operation 只允许 park，不得自动 redo；后续如要观察或继续，必须进入显式的人机恢复流程。
...
恢复时的 workspace root 必须来自 source Run，而不能由 prepared fact 自证。Runtime 先独立
canonicalize `sourceRun.cwd`，要求它与 fact 中的 `workspaceRoot` 完全一致，再以该可信 root
解析 operation args 并核对 `canonicalPath`；任何不一致都在 inspect/apply 前 park。worker 的
`cwd` 与 permission mode 同样只能来自 source Run，禁止回退使用 fact 提供的 root 或隐式默认
权限。这样即使单条 prepared fact 被篡改为另一个 workspace，也不能扩大 worker capability。

所有 checkpoint observation 都拒绝 symlink。支持 `O_NOFOLLOW` 的平台在 open 时直接禁止跟随；
其他平台执行 `lstat → open/fstat → bounded read → fstat/lstat`，并核对 file identity 与读期间
metadata 稳定性。target 在 T1 后被替换为指向 workspace 内或外的 symlink 时都必须 park，不能因
referent 恰好具有 expected-after hash 而合成成功。
...
当 recovery 首次观察就得到 expected-after 时，也必须先通过同一个 worker 执行幂等
`finalize`：只删除 hash 与 durable fact 匹配的 operation temp/before backup，fsync parent
成功后才允许写 synthesized response。finalize 不得盲删同名文件；清理失败或 worker 结果不确定
时保持 operation unsettled，下一次 authoritative recovery 重新判定。
...
- source Run cwd 必须独立 canonicalize 并与 fact workspaceRoot、args 解析出的 canonical target 三方一致；fact 不得自行决定 worker cwd 或权限模式；
...
- recovery 观察使用 no-follow、bounded、handle/path identity revalidation；首次观察已为 after 时仍先在 worker 内幂等清理 verified temp/Windows before backup，再合成 response；
