...
Phase 1-8 已完成。macOS 本地 managed runtime 已具备以下完整链路：
...
这表示 macOS 的“权限管理 + OS sandbox 兜底”主链路已经完成。它不表示 Codex sandbox 的全部能力都已复刻；Phase 9 diagnostics、Linux backend、managed network proxy 和 unsandboxed retry 仍未实现。
...
用户选择权限模式后，Maka 会先形成明确的 filesystem/network profile。runtime 为当前 session 构造命令执行器、文件 worker 和 capability snapshot；工具调用先经过 capability gate 与现有审批矩阵，再进入对应执行边界。
...
| diagnostics/model context | Phase 9 | 未实现 |
...
### Phase 9: diagnostics 与模型上下文

- 向 runtime diagnostics 暴露 active profile、workspace roots、sandbox type 和 capability reason。
- 给模型提供准确、稳定的权限边界描述。
- 不泄露完整 SBPL、argv、env、文件内容或 edit strings。
...
在 Codex 外层 sandbox 中直接运行嵌套 `sandbox-exec` 或本地监听测试会被外层环境拒绝；相关 macOS/desktop 测试已在允许嵌套 Seatbelt和本地端口的环境中重跑。
