...
- `notes/alma-memory-reverse-2026-05-25.md` (kenji msg `66fd3eab`) — Alma facts + Maka quasi-memory inventory.
...
## What NOT to copy from Alma (negative reference list)

Per `notes/alma-memory-reverse-2026-05-25.md`:
...
6. **Unauthenticated local route** (Alma's `05-express-api.md` exposes memory over local HTTP) — Maka memory MUST NOT have a local HTTP endpoint; if Local Gateway lane lands, it MUST go through capability scope allowlist + audit log, never default-on.
...
8. **Soul tree / `~/.config/alma` long-term file** — out of v1 scope. If durable persistence lands (PR-MEMORY-2+), it sits inside the existing workspace store, not a separate global tree.
