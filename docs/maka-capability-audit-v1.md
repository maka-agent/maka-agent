...
Reference app / Reference app 那类成熟工作台时必须补齐的系统 contract。
...
- Reference app 本地资料：`~/Downloads/reference-source/STUDY-GUIDE.md`,
  `~/Downloads/reference-source/docs/00-GAP-ANALYSIS.md`, `docs/08-memory.md`,
  `docs/11-providers.md`, `docs/17-workspaces-git.md`, `docs/39-streaming-deep.md`,
  `readable/preload.js`
- Reference app 本地资料：`~/.reference app-agent/docs/reference app-cli.md`,
  `~/.reference app-agent/release-notes/*.md`
...
### Reference app / Reference app 对照

Reference app preload 已有 `snapshot.*` 一组能力（create / file / list / get / diff / rollback / cleanup），并把 workspace diff/snapshot 从 transcript 中分离出来。
Reference app 的 release notes 也反复强调 diff / preview / data table / terminal preview 的独立呈现。

可复制的是：artifact 作为第一等对象；不可照搬的是 Reference app wide-open local API。
...
Reference app streaming docs 强调 tool output delta；Reference app release notes 也多次修复 partial output / streaming error recovery。
...
Reference app provider 层会根据 model id 补 capability；Reference app 在 release notes 中也反复修 model resolution / Bedrock region / unsupported model 过滤。
...
Reference app 有 thread/workspace/snapshot 体系；Reference app 有 workspace、labels、sources、automations，release notes 里也提到 branch/session recovery。
...
Reference app 有 snapshots / branch / compact；Reference app release notes 明确 session branching 和 branch cutoff 是长期复杂点。
...
Reference app 有大量 `/api/*/status` 和 settings tree；Reference app CLI 的 `source test/validate`、automation validate/lint 都是 health surface。
...
Reference app CLI has first-class `source`, `skill`, `automation`, `permission` entities with validate/test/history commands.
...
Reference app `validate-server` auto-bootstraps temp workspace/connection for validation；Reference app onboarding has permission timing and settings tree.
...
Do not jump directly to Reference app's sqlite-vec + ONNX. The first missing piece is user-visible memory CRUD and retrieval contract.

### Reference app 对照

Reference app has memory store, embeddings, sleep cycle, Recall tool, memory archive. Useful structure:
...
From Reference app/Reference app, do not copy these patterns into Maka:
